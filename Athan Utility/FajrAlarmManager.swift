...
class FajrAlarmManager {
    ...
    private func knownAlarms(manager: AlarmManager) -> [UUID: Alarm]? {
        guard let alarms = try? manager.alarms else {
            NSLog("ATHAN-ALARM: AlarmManager.alarms query FAILED — assuming stored IDs are still live")
            return nil
        }
        var byID: [UUID: Alarm] = [:]
        for alarm in alarms { byID[alarm.id] = alarm }
        // Logged so a run from Xcode can confirm the alarms really landed with the
        // system, rather than inferring it from our own stored IDs.
        NSLog("ATHAN-ALARM: system reports %d alarm(s): %@", byID.count,
              byID.keys.map { $0.uuidString.prefix(8) }.joined(separator: ", "))
        return byID
    }
    ...
    private func makeConfiguration(
        ...
        // Live Activities are intentionally disabled app-wide (product
        // decision). AlarmKit only requires a registered Live Activity when the
        // alarm uses `secondaryButtonBehavior: .countdown` (the snooze
        // countdown that renders on the Lock Screen / Dynamic Island). To keep
        // the alarm working without any Live Activity, we always build an
        // alert-only presentation: a single Stop button, no snooze countdown.
        // `snoozeEnabled` is accepted for call-site compatibility but no longer
        // adds a countdown secondary button. If Live Activities are re-enabled
        // later, restore the `.countdown` snooze branch here.
        _ = snoozeEnabled   // silence unused-parameter warning; see note above
        ...
        // No countdown duration: the snooze countdown requires a Live Activity
        // (disabled app-wide), so the alarm is a plain alert with a Stop button.
        let countdownDuration: Alarm.CountdownDuration? = nil
        _ = snoozeMinutes   // unused while snooze countdown is disabled
