    ...
    private var scrollBody: some View {
        ScrollView(showsIndicators: true) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                offsetSection
                daysSection
                // Snooze is hidden: the snooze countdown needs a Live Activity,
                // which is disabled app-wide. See FajrAlarmManager.makeConfiguration.
                // snoozeSection
    ...
    private func weekdayChip(index: Int) -> some View {
        ...
        Button(action: {
            ...
            VStack(spacing: 5) {
                ...
                // Clear on/off affordance under the letter: a bold filled
                // checkmark circle when selected, a hollow circle when not.
                Image(systemName: isOn ? "checkmark.circle.fill" : "circle")
                    .font(.system(size: 13, weight: .bold))
                    .foregroundColor(isOn ? .black : .white.opacity(0.55))
            }
            .frame(maxWidth: .infinity)
            .frame(minHeight: 48) // min, not fixed: grows with Dynamic Type
            .padding(.vertical, 4)
            .background(
